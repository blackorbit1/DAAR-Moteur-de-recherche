...
public class EPSdfa {
    ...
    private boolean getFinal(Integer ligne_num, int[][] transiMatrix,
    	...
    	// un etat final reste final 
		if(ndfa_sortant.get(ligne_num).get(258).get(0) == 1) return true;
		
    	int sumTM = 0;
    	for(int i : transiMatrix[ligne_num]) sumTM += i;
    	if(sumTM == 1 || ndfa_sortant.get(ligne_num).get(256) == null) {
    		return false;
    	} // un etat sans aucune transition sortante ou 
    	// pour lequel chaque transition sortante n'est pas epsilon
    	// ne change pas de finalite : on a elimine les etats finaux donc 
    	// un tel etat est forcement non final 
		
    	
    	// pour chaque transition sortante possible
    	for(int i = 0; i < transiMatrix.length; i++) {
    		// si cette transition existe 
    		if (transiMatrix[ligne_num][i] == 1 && i != ligne_num){
    			ArrayList<Integer> ndfasi = ndfa_sortant.get(ligne_num).get(256);
    			for(int j = 0; j < ndfasi.size(); j++) {
    	    		if(ndfasi.get(j) != ligne_num) {
    	    			// on verifie si l'etat vers lequel la transition 
    	    			// (ou un de ses descendants par chaine de transitions epsilon)
    	    			// est final
    	    			boolean res0 = getFinal(ndfasi.get(j), transiMatrix, ndfa_sortant);
    	    			// on s'arrete au premier etat final trouve pour optimiser legerement
    	    			if(res0) return true;
    	    		}
    			}
    		}
    		...
    		if (transiMatrix[ligne_num][i] == 1){
				...
				for(int j = 0; j < ndfa_sortant.get(ligne_num).size(); j++) {
					...
					if(ndfa_sortant.get(ligne_num).get(j) != null) {
						for(Integer elt : ndfa_sortant.get(ligne_num).get(j)) {
							val = j;
							key = elt;
							if(val == 256 ) {
								if(ligne_num != i)
